...
int main() {
    ...
    memset(&echo_client, 0, sizeof(echo_client));
	  ...
	  echo_server.sin_port = htons(LOWER_PORT); //specificies the port number
    ...
    while (1) {
      ...
      cout << "Received " << org << " from client\n";
	    ...
	    bytes = sendto(echo_socket, low, strlen(low), 0, client, len);
      if (bytes == -1){
